      ...
      >
        ...
        <Content>
          ...
          <div style={{ padding: "4px 0 16px" }}>
            <BodyText>
              Relate seus sintomas antes de sair de casa e receba uma
            ...
            >
              ...
              <span style={{ fontSize: 12, color: "#4E6A80", lineHeight: 1.4 }}>
                A triagem pode ser feita de forma anônima, sem necessidade de
                cadastro ou login.
              </span>
            </div>
          </div>
      ...
      <Content>
        ...
        >
          <p style={{ margin: "0 0 8px" }}>
            <strong>Por que coletamos dados?</strong> Para processar sua triagem
            e gerar recomendações de atendimento. Dados de saúde são
            considerados sensíveis pela LGPD (Lei 13.709/2018) e tratados com
            proteção reforçada.
          </p>
          <p style={{ margin: "0 0 8px" }}>
            <strong>O que coletamos:</strong> Sintomas relatados, localização
            aproximada (para busca de unidades) e dados de perfil, se fornecidos
            voluntariamente.
          </p>
          <p style={{ margin: "0 0 8px" }}>
            <strong>Compartilhamento:</strong> Dados nunca são vendidos. Podem
            ser compartilhados anonimamente com a Secretaria de Saúde para fins
            de vigilância epidemiológica.
          </p>
          <p style={{ margin: 0 }}>
            <strong>Triagem anônima:</strong> Se não criar conta, nenhum dado
            pessoal identificável é armazenado após a sessão.
          </p>
        </div>

        <div style={{ marginBottom: 16 }}>
          <div onClick={() => setAgreed(!agreed)} style={{ cursor: "pointer" }}>
            <CheckboxItem
...
export function Q1Screen({ navigate }: { navigate: Navigate }) {
  ...
  return (
    <ScreenWrap>
      <NavBar
        title="Triagem"
        onBack={() => navigate(hasAccount ? "home" : "lgpd")}
      />
      ...
      <Content>
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 12, color: "#7C93A6" }}>
            Selecione o que melhor descreve sua queixa
          </div>
        </div>
        ...
        {SYMPTOMS.map((s) => (
          <OptionItem
            key={s}
            label={s}
            selected={answers.symptom === s}
            onClick={() => updateAnswers({ symptom: s })}
          />
        ))}
        ...
        <div style={{ marginTop: 8 }}>
          <Btn
            label="Próxima →"
            onClick={() => canProceed && navigate("q2")}
    ...
    <ScreenWrap>
      ...
      <Content>
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 12, color: "#7C93A6" }}>
            Considere o início do sintoma principal
          </div>
        </div>
        ...
        {DURATIONS.map((d) => (
          <OptionItem
            key={d}
            label={d}
            selected={answers.duration === d}
            onClick={() => updateAnswers({ duration: d })}
          />
        ))}
        ...
        <div style={{ marginTop: 8 }}>
          <Btn
            label="Próxima →"
            onClick={() => answers.duration && navigate("q3")}
            variant={answers.duration ? "primary" : "ghost"}
          />
        </div>
      </Content>
    </ScreenWrap>
...
export function Q3Screen({ navigate }: { navigate: Navigate }) {
  ...
  const allAnswered =
    answers.fever && answers.intensePain && answers.breathingDifficulty
  ...
  return (
    <ScreenWrap>
      ...
      <Content>
        ...
        {[
          {
            label: "Tem febre (acima de 37,8°C)?",
            val: answers.fever,
            key: "fever" as const,
          },

          {
            label: "Sente dor intensa (≥7 em 10)?",
            val: answers.intensePain,
            key: "intensePain" as const,
          },

          {
            label: "Tem dificuldade para respirar / falta de ar?",
            val: answers.breathingDifficulty,
            key: "breathingDifficulty" as const,
          },
        ].map(({ label, val, key }) => (
          <div key={label} style={{ marginBottom: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <OptionItem
                label="Sim"
                selected={val === "sim"}
                onClick={() => updateAnswers({ [key]: "sim" })}
              />
              <OptionItem
                label="Não"
                selected={val === "nao"}
                onClick={() => updateAnswers({ [key]: "nao" })}
              />
            </div>
          </div>
        ))}
        ...
        <div style={{ marginTop: 8 }}>
          <Btn
            label="Próxima →"
            onClick={() => allAnswered && navigate("q4")}
  ...
  return (
    <ScreenWrap>
      ...
      <Content>
        ...
        >
          <OptionItem
            label="Sim — piorou rápido"
            selected={answers.rapidWorsening === "sim"}
            onClick={() => updateAnswers({ rapidWorsening: "sim" })}
          />
          <OptionItem
            label="Não — estável ou melhorando"
            selected={answers.rapidWorsening === "nao"}
        ...
        <Btn
          label="Próxima →"
          onClick={() => answers.rapidWorsening && navigate("q5")}
          variant={answers.rapidWorsening ? "primary" : "ghost"}
        />
      </Content>
    </ScreenWrap>
  )
    ...
    <ScreenWrap>
      ...
      <Content>
        ...
        {GROUPS.map((g) => (
          <div
            key={g}
            onClick={() => toggleVulnerableGroup(g, NONE_LABEL)}
            style={{ cursor: "pointer", marginBottom: 6 }}
          >
            <CheckboxItem
              label={g}
              checked={answers.vulnerableGroups.includes(g)}
            />
          </div>
        ))}
    ...
    <ScreenWrap>
      ...
      <Content>
        ...
        >
          <OptionItem
            label="Sim"
            selected={answers.usingMedication === "sim"}
            onClick={() => updateAnswers({ usingMedication: "sim" })}
          />
          <OptionItem
            label="Não"
            selected={answers.usingMedication === "nao"}
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <Btn
                label="Confirmar e ver resultado"
                onClick={() => navigate("result")}
                variant="primary"
              />
              <Btn
                label="Voltar e revisar respostas"
