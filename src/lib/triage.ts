...
const CHRONIC_GROUP = 'Portador de doença crônica (diabetes, HAS, cardiopatia, DPOC...)'
...
export function classifyRisk(a: TriageAnswers): TriageResult {
  ...
  const hasHighRiskGroup = a.vulnerableGroups.some((g) => HIGH_RISK_GROUPS.includes(g))
  const hasChronic = a.vulnerableGroups.includes(CHRONIC_GROUP)
  const recentOnset = a.duration !== null && RECENT_DURATIONS.includes(a.duration)
  ...
  if (a.symptom === 'Dor no peito' && (a.breathingDifficulty === 'sim' || a.intensePain === 'sim')) {
    reasons.push('Dor no peito associada a falta de ar ou dor intensa — sinais compatíveis com emergência cardiovascular.')
    return { level: 'red', reasons }
  }
  if (a.breathingDifficulty === 'sim' && a.rapidWorsening === 'sim') {
    reasons.push('Falta de ar com piora rápida nas últimas horas — risco respiratório grave.')
    return { level: 'red', reasons }
  }
  ...
  if (a.symptom === 'Dor no peito') {
    reasons.push('Dor no peito relatada — necessita avaliação rápida mesmo sem outros sinais.')
    return { level: 'orange', reasons }
  }
  ...
  if (hasHighRiskGroup && (a.fever === 'sim' || a.rapidWorsening === 'sim')) {
    reasons.push('Paciente em grupo de risco (criança, idoso, gestante, puérpera ou imunossuprimido) com febre ou piora rápida.')
    return { level: 'orange', reasons }
  }
  if (isGI && recentOnset && (a.fever === 'sim' || hasHighRiskGroup)) {
    reasons.push('Vômito ou diarreia de início recente, com risco de desidratação.')
    return { level: 'orange', reasons }
  }
  ...
  if (hasHighRiskGroup) {
    reasons.push('Paciente pertence a grupo de risco e deve ser priorizado, mesmo sem sinais de alarme.')
    return { level: 'yellow', reasons }
  }
  ...
  if (a.fever === 'sim' || a.symptom === 'Dor abdominal' || hasChronic) {
    reasons.push('Sintomas presentes, sem sinais de alarme — recomenda-se avaliação em UBS.')
    return { level: 'green', reasons }
  }
  if (longOnset) {
    reasons.push('Sintomas persistentes há vários dias — recomenda-se avaliação em UBS.')
    return { level: 'green', reasons }
  }
